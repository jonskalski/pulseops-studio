...
- [ ] Re-run published cluster posts with new voice — pipeline has no rewrite mode; needs a --rewrite flag that fetches existing post, passes to draft/polish as refresh, PATCHes existing post ID instead of creating new
- [ ] POPS-24: Investigate featured snippet paragraph missing from published posts — draft agent instructs a 40-60 word direct answer paragraph after intro but spot check of 10 recent posts shows none have it; likely needs enforcement in validate_and_repolish() like title/meta desc
